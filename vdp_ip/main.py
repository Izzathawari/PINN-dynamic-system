...
def main ():
    ...
    t_timedata = np.arange(0, FULL_DATA_LENGTH + DELTA_T / 2, DELTA_T)
    print(f"\n-----Preparing Van der Pol Ground Truth Data (0s to {FULL_DATA_LENGTH}s)-----")
    ...
    train_lengths = config.get("train data")

    for train_length in train_lengths:

        print(f"\n==================================================")
        print(f" Running Experiment: TRAIN_LENGTH = {train_length}s")
        print(f"==================================================")

        TRAIN_LENGTH = train_length

        save_filename = output_dir/f"prediction_{train_length}.png"

        t_full_np = np.arange(0, FULL_DATA_LENGTH + DELTA_T / 2, DELTA_T)

        t_train_np = t_full_np[t_full_np <= TRAIN_LENGTH]
        t_test_np  = t_full_np[t_full_np >= TRAIN_LENGTH]
